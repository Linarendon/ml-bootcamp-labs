# --- App Streamlit para Diagramas de Rosa de orientaciones (paleocorrientes o estratificación) ---
...
bins = st.slider('Número de bins (divisiones angulares)', min_value=4, max_value=36, value=12)
...
if mostrar_promedio:
    r_max = ax.get_rmax()
    # Invertir el sentido de la flecha 180°
    ax.arrow(
...
import scipy.signal as signal
    ...
    # --- Calcular histograma angular ---
    angles = df_f['Azimuth-dega'].dropna().values
    hist, bin_edges = np.histogram(angles, bins=36, range=(0, 360))
    bin_centers = (bin_edges[:-1] + bin_edges[1:]) / 2

    # --- Detectar picos significativos ---
    peaks, _ = signal.find_peaks(hist, height=np.max(hist) * 0.4)  # 40% del máximo
    direcciones_principales = bin_centers[peaks]

    # --- Dibujar flechas ---
    r_max = ax.get_rmax()
    for i, ang in enumerate(direcciones_principales):
    ax.arrow(
        np.deg2rad((ang + 180) % 360), 0, 0, r_max * 0.8,
        width=0.05, color=['red', 'orange', 'purple'][i % 3],
        alpha=0.8, zorder=10, length_includes_head=True, head_width=0.15
    )

    # --- Mostrar direcciones en el título ---
    titulo += "\nTendencias dominantes: " + ", ".join(f"{ang:.1f}°" for ang in direcciones_principales)
    # --- Título y subtítulo ---
    plt.title(
        f"{titulo}\n{profundidad_min:.2f}–{profundidad_max:.2f} ft\n"
        f"Dirección promedio: {mean_angle_deg:.1f}°  |  Concentración R={R:.2f}",
        pad=30
    )
    ...
    st.download_button(
        ...
        data=buffer,
        file_name=f"diagrama_rosa_{pozo_sel}_{tipo_sel}_{profundidad_min:.0f}-{profundidad_max:.0f}ft.png",
        mime="image/png"
    )
