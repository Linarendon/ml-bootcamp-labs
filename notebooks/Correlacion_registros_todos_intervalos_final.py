# ==========================================================
# 🔥 Correlacion_heatmap_consistencia_app_v4.py
# ==========================================================
...
def expected_sign(reg, inv):
    ...
    if r == "GR" and i in ("IP", "VPVS", "XPLT"): return "Positiva"
    if r == "VSH" and i in ("IP", "VPVS", "XPLT"): return "Positiva"
    if r in ("PERM", "PERMEABILIDAD") and i in ("IP", "VPVS", "XPLT"): return "Negativa"
    if r in ("POR", "POROSIDAD", "POROSITY") and i in ("IP", "VPVS", "XPLT"): return "Negativa"
    if r.startswith("GR"): return "Positiva"
    if r.startswith("VSH"): return "Positiva"
    if r.startswith("PER") or r.startswith("K"): return "Negativa"
    if r.startswith("POR"): return "Negativa"
    return "N/A"
...
def build_annot_string(corr_val, cons_flag, exp_sign, style="num_sym"):
    ...
    if style == "num": return num
    if style == "num_sym": return f"{num}\n{symbol}"
    if style == "num_sym_rule": return f"{num}\n{symbol}\n({exp_sign})"
    return num
...
# ==========================================================
# 3️⃣ Selección de intervalo (modificada para múltiples)
# ==========================================================
if "INTERVALO" in df.columns:
    intervalos = sorted(df["INTERVALO"].dropna().unique().tolist())
    intervalos_sel = st.sidebar.multiselect("🪨 Selecciona uno o varios intervalos", intervalos, default=intervalos)
    df = df[df["INTERVALO"].isin(intervalos_sel)].copy()
    st.sidebar.success(f"Intervalos seleccionados: {', '.join(intervalos_sel)}")
else:
...
ax.set_title(f"Heatmap: Correlaciones por Pozo ({', '.join(intervalos_sel)})")
...
st.download_button("📥 Descargar Heatmap (JPEG)", data=buf,
                   file_name=f"Heatmap_{'_'.join(intervalos_sel)}.jpg", mime="image/jpeg")

# ==========================================================
# 11️⃣ Resumen de consistencia
# ==========================================================
...
if not ranking.empty:
    st.success(f"✨ Pozo con mejor correlación en {', '.join(intervalos_sel)}: **{ranking.iloc[0,0]}**")

# ==========================================================
# 13️⃣ Comparativo global
# ==========================================================
...
if "INTERVALO" in df.columns:
    ...
    sns.barplot(data=df_summary[df_summary["INTERVALO"].isin(intervalos_sel)],
                x="INTERVALO", y="CORRELACION_MEDIA_ABS", hue="POZO", ax=ax)
...
if "INTERVALO" in df.columns:
    ...
    if pozos_sel:
        ...
        for pozo in pozos_sel:
            datos_pozo = df_summary[(df_summary["POZO"] == pozo) & (df_summary["INTERVALO"].isin(intervalos_sel))]
        ...
        fig_radar.update_layout(
            ...
            showlegend=True,
            title=f"Comparación de desempeño entre pozos ({', '.join(intervalos_sel)})"
        )
