...
uploaded = st.sidebar.file_uploader("Sube el archivo CSV (separado por ';')", type=["csv"])

if uploaded is not None:
    df = pd.read_csv(uploaded, sep=";")
else:
    st.sidebar.info("Sube tu archivo CSV para comenzar.")
    st.stop()
...
for col in ["POZO", "CURVA_REGISTRO", "CURVA_INVERSION", "CORRELACION", "CONSISTENCIA"]:
    if col not in df.columns:
        st.sidebar.error(f"Falta la columna requerida: {col}")
        st.stop()

# -----------------------------
# Filtros por curvas
# -----------------------------
st.sidebar.subheader("🎚️ Filtros de curvas")

# Filtro para curvas de registro
curvas_registro = sorted(df["CURVA_REGISTRO"].dropna().unique())
curvas_reg_sel = st.sidebar.multiselect(
...
curvas_inv_sel = st.sidebar.multiselect(
...
if df.empty:
    st.sidebar.warning("⚠️ No hay datos con las curvas seleccionadas. Ajusta los filtros.")
    st.stop()
...
if "INTERVALO" in df.columns:
    ...
    seleccion = st.sidebar.multiselect("Selecciona uno o más intervalos a analizar:", intervalos, default=intervalos)
    df = df[df["INTERVALO"].isin(seleccion)]
else:
    st.sidebar.info("No se encontró la columna 'INTERVALO', se usarán todos los datos disponibles.")
...
if not summary.empty:
    st.sidebar.subheader("🔥 Heatmap de correlación absoluta promedio")
    pivot = summary.pivot(index="CURVA_REGISTRO", columns="CURVA_INVERSION", values="mean_abs")

    if pivot.empty:
        st.sidebar.warning("No hay suficientes datos para generar el heatmap.")
    else:
